...
def sign_provenance(track: str, claims: dict[str, Any]) -> str:
    ...
    public_der = subprocess.run(
        ...
        input=private_key.encode(), stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True,
    ).stdout
    ...
    signing_input = f"{base64url(json.dumps(header, sort_keys=True, separators=(',', ':')).encode())}.{base64url(json.dumps(claims, sort_keys=True, separators=(',', ':')).encode())}"
    import tempfile
...
def create_provenance(args: argparse.Namespace, version: str, manifest: Artifact, application: Artifact, path: Path, order: int) -> tuple[Artifact, str]:
    claims = {
        "schema": "rad.firmware.provenance.v1", "issuer": "research-and-desire",
        "track": args.track, "deviceType": args.device_type,
        "hardwareVariant": getattr(args, "hardware_variant", "default"), "kind": args.kind,
        "version": version, "buildSha": args.build_sha, "manifestSha256": manifest.sha256,
        "applicationSha256": application.sha256, "runtimeImageSha256": runtime_image_sha256(application),
...
def publish(args: argparse.Namespace) -> str:
    ...
    release_artifacts = sorted(
        [*installable, *web_installers], key=lambda item: item.install_order
    )
